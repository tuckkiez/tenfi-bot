...
bot_token = '2068478237:AAHQriIQDwxNbL1UEHENp7_BgsLhQ52R_xk'
...
@bot.message_handler(commands=['xxxxxxxxx'])
...
def send_welcome(message):
  ...
  bot.send_message(chatid, 'TEN มีอะไรมาใหม่บ้างในช่วงนี้? 🗺\n'
...
def send_welcome(message):
  ...
  bot.send_message(chatid, 
    ...
    'สำหรับ TENFI LP ให้นำจำนวน TENFI ใน LP x2 = จำนวน TENFI ที่จะรับสิทธิ์ใน TEN Lots โดยนับจำนวน TENFI ในวันที่กดรับสิทธิ์ TEN Lots ไม่ได้นับตั้งแต่วันที่ผูก LP\n\n'
    ...
    'ถ้า Claim reward จาก Ten Lots เมื่อไหร่ จะเริ่มนับวันสะสมใหม่เป็นวันที่ 1 และ reward สะสม จะเริ่มนับ 0 ใหม่\n\n'
    'ใครกดรับสิทธิ์ Ten Lots ก่อนจะได้รับ reward สะสมจาก Ten Lots ก่อน\n'
    'และเริ่มนับจำนวนวันที่สะสมก่อน\n\n'
    'การ Claim reward จาก Ten Lots\n'
    'Example 1\n'
    'ณ วันที่ 95 สะสม reward ได้ 200$\n'
    'Claim ได้ 50% = 100$ .. อีก 100$ มอบให้ Dev\n'
    'reward สะสมจะกลายเป็น 0 และเริ่มนับวันที่สะสมใหม่เป็นวันที่ 1\n\n'
    'Example 2\n'
    'ณ วันที่ 1 สะสม reward ได้ 10$\n'
    'Claim ได้ 25% = 2.5$ .. อีก 7.5$ มอบให้ Dev\n'
